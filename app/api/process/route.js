...
export async function POST(req, res) {
  ...
  const weeklyRosterFlag = await searchParams.get("weeklyRosterFlag"); // New flag
  ...
  function assignShift(availablePeople, rolePeriod, rosterDate, shiftsAssigned, role, doubleShiftFlag) {
    ...
    // Function to generate roster for a week (modified to handle weeklyRosterFlag)
    function generateRoster(
      ...
      return new Promise((resolve) => {
        ...
        for (let week = 1; week < parseInt(numberOfWeeks, 10) + 1; week++) {
          ...
          if (weeklyRosterFlag === 'true') {
            // Weekly granularity
            const weekStartDate = currentWeekStartDate.toFormat("EEEE dd LLLL yyyy");
            ...
            uniqueRoles.forEach((roleObj) => {
              ...
              if (roleObj.isAllDay) {
                if (!roster[weekStartDate][role]) {
                  roster[weekStartDate][role] = [];
                }
  
                const availablePeopleForRoleAllDay = rosterData.filter(
                  (person) =>
                    person.roles.includes(role) &&
                    isAvailable(person, currentWeekStartDate, role, week) &&
                    isAvailableHalfDay(person, currentWeekStartDate, week, "AM") &&
                    isAvailableHalfDay(person, currentWeekStartDate, week, "PM")
                );
                ...
                if (availablePeopleForRoleAllDay.length > 0) {
                  ...
                  roster[weekStartDate][role].push(personForAllDayShift.name);
                  shiftsAssigned.set(
                ...
                if (!roster[weekStartDate][`morning_${role}`]) {
                  roster[weekStartDate][`morning_${role}`] = [];
                }
                if (!roster[weekStartDate][`afternoon_${role}`]) {
                  roster[weekStartDate][`afternoon_${role}`] = [];
                }
                ...
                assignShift(availablePeopleForRoleAM, 'morning', roster[weekStartDate], shiftsAssigned, role, doubleShiftFlag);
                assignShift(availablePeopleForRolePM, 'afternoon', roster[weekStartDate], shiftsAssigned, role, doubleShiftFlag);
              }
            });
